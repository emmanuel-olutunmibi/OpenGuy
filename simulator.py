...
class RobotSimulator:
    ...
    def __init__(self, use_3d: bool = True):
        ...
        self.use_3d = False  # Force 2D for now, PyBullet setup for future
        self.x: float = 0.0
        self.y: float = 0.0
    ...
    def _init_pybullet(self):
        ...
        # Create gripper as separate body (simplified)
    ...
    def _execute_move(self, direction: str, distance: float) -> str:
        """Execute a movement command and return status message."""
        print(f"DEBUG: _execute_move called with direction={direction}, distance={distance}")
        if distance <= 0:
            raise ValueError(f"Distance must be positive, got {distance}")
        if distance > 100:
            raise ValueError(f"Distance too large (max 100 cm), got {distance}")
